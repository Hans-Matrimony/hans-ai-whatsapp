...
# ===================================================================
# Daily Horoscope Task - Runs at 8:30 AM IST daily
# ===================================================================

@celery_app.task(bind=True, max_retries=2, default_retry_delay=300)
def daily_horoscope_task(self):
    """
    Send daily horoscope to all users with birth details at 8:30 AM IST.
    ...
    try:
        ...
        # STRICT TIME CHECK: Only run at exactly 8:30 AM IST (with 15-minute grace window)
        # This ensures the task only runs once per day at the scheduled time
        is_eight_thirty = (current_hour == 8 and 30 <= current_minute < 45)

        # Allow test mode to bypass time check
        test_mode = os.getenv("DAILY_HOROSCOPE_TEST_NUMBER")

        if not is_eight_thirty and not test_mode:
            logger.info(f"[Daily Horoscope] Not 8:30 AM (currently {current_hour}:{current_minute:02d} IST), skipping")
